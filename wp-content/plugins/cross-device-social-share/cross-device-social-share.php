...
/*
  ...
  Description: Simple Facebook, Twitter and Pinterest share buttons with cross-devices link sending tool
  Version: 1.2
  Author: inout
  Text Domain: cross-device-social-share
  License: GPLv2 or later
  Credits: This plugin was derived from AccessPress Social Share by AccessPress Themes (https://accesspressthemes.com/)
 */
...
if ( !defined( 'APSS_TEXT_DOMAIN' ) ) {
	define( 'APSS_TEXT_DOMAIN', 'cross-device-social-share' );
}

if ( !defined( 'APSS_SETTING_NAME' ) ) {
	define( 'APSS_SETTING_NAME', 'cross_share_settings' );
}
...
if ( !class_exists( 'APSS_Class' ) ) {
	...
	class APSS_Class {
		...
		function plugin_text_domain() {
			load_plugin_textdomain( 'cross-device-social-share', false, APSS_LANG_DIR );
		}
		...
		function register_admin_assets() {
			 ...
			 * Backend CSS
			 * */
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'cross-device-social-share' ) {
				wp_enqueue_style( 'aps-admin-css', APSS_CSS_DIR . '/backend.css', false, APSS_VERSION ); //registering plugin admin css
				wp_enqueue_style( 'fontawesome-css', '//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.5.0/css/font-awesome.min.css', false, APSS_VERSION );
		...
		function register_frontend_assets() {
			wp_enqueue_style( 'apss-font-awesome', '//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.5.0/css/font-awesome.min.css', array(), APSS_VERSION );
		...
		function add_apss_menu() {
			add_menu_page( 'Social Share', 'Social Share', 'manage_options', 'cross-device-social-share', array( $this, 'main_page' ), APSS_IMAGE_DIR . '/apss-icon.png' );
			//add_submenu_page( 'cross-device-social-share', __( 'SAccessPress Social Share', 'cross-device-social-share' ), __( 'Social Icons Settings', 'cross-device-social-share' ), 'manage_options', 'cross-device-social-share', array( $this, 'main_page' ) );
		}
		...
		function apss_restore_default_settings() {
			...
			if ( !empty( $_GET ) && wp_verify_nonce( $nonce, 'apss-restore-default-settings-nonce' ) ) {
				...
				include( 'inc/backend/activation.php' );
				$_SESSION['apss_message'] = __( 'Settings restored Successfully.', 'cross-device-social-share' );
		...
		function apss_clear_cache() {
			if ( !empty( $_GET ) && wp_verify_nonce( $_GET['_wpnonce'], 'apss-clear-cache-nonce' ) ) {
				...
				$_SESSION['apss_message'] = __( 'Cache cleared Successfully', 'cross-device-social-share' );
				wp_redirect( admin_url() . 'admin.php?page=cross-device-social-share' );
			}
		}
		...
		function social_meta_box() {
			add_meta_box( 'ap-share-box', 'Social share options', array( $this, 'metabox_callback' ), '', 'side', 'core' );
		}
		function metabox_callback( $post ) {
			wp_nonce_field( 'save_meta_values', 'ap_share_meta_nonce' );
			$content_flag = get_post_meta( $post->ID, 'apss_content_flag', true );
			?>
			<label><input type="checkbox" value="1" name="apss_content_flag" <?php checked( $content_flag, true ) ?>/><?php _e( 'Hide share icons in content', 'cross-device-social-share' ); ?></label><br>
			<?php
		}
